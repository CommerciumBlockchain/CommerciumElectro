...
from commerciumelectro.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from commerciumelectro import bitcoin
from commerciumelectro.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        try:
            ...
            self.app.show_info(_("Not a CommerciumElectro URI"))
            return
    ...
    def do_save(self):
        ...
        from commerciumelectro.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(
                    _('Recipient not specified.') + ' ' + _('Please scan a CommerciumElectro address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid CommerciumElectro Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from commerciumelectro.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share CommerciumElectro Request"))
...
class AddressScreen(CScreen):
    ...
    def ext_search(self, card, search):
        search = search[:-4] if search.endswith("CMM") else search
