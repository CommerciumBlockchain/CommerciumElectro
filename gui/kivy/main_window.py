...
from commerciumelectro.paymentrequest import InvoiceStore
from commerciumelectro.util import profiler, InvalidPassword, NotEnoughFunds
from commerciumelectro.plugins import run_hook
from commerciumelectro.util import format_satoshis, format_satoshis_plain
from commerciumelectro.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED
...
Factory.register('TabbedCarousel', module='commerciumelectro_gui.kivy.uix.screens')
...
class ElectrumWindow(App):
    ...
    def choose_server_dialog(self, popup):
        ...
        def cb2(host):
            from commerciumelectro.bitcoin import NetworkConstants
    ...
    def on_new_intent(self, intent):
        if intent.getScheme() != 'commerciumelectro':
            return
    ...
    def on_qr(self, data):
        from commerciumelectro.bitcoin import base_decode, is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
        if data.startswith('cmm:'):
            self.set_URI(data)
            return
        # try to decode transaction
        from commerciumelectro.transaction import Transaction
        from commerciumelectro.util import bh2u
        try:
    ...
    def show_pr_details(self, req, status, is_invoice):
        from commerciumelectro.util import format_time
    ...
    def load_wallet_by_name(self, path):
        ...
        if wallet:
            ...
            Logger.info('CommerciumElectro: Wallet not found. Launching install wizard,')
    ...
    def init_ui(self):
        ...
        Factory.register('AnimatedPopup',
                         module='commerciumelectro_gui.kivy.uix.dialogs')
        Factory.register('QRCodeWidget',
                         module='commerciumelectro_gui.kivy.uix.qrcodewidget')
        ...
        self.icon = "icons/commerciumelectro.png"
