...
from commerciumelectro.util import format_satoshis, set_verbosity
from commerciumelectro.bitcoin import is_address, COIN, TYPE_ADDRESS
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'commerciumelectro create'")
            exit()
    ...
    def do_send(self):
        if not is_address(self.str_recipient):
            self.show_message(_('Invalid CommerciumElectro address'))
            return
        try:
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy'):
                proxy = commerciumelectro.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
