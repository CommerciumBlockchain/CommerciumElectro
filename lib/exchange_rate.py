...
class CoinFloor(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('webapi.coinfloor.co.uk:8090/bist/CMM/GBP', '/ticker/')
...
class CEXIO(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('cex.io', '/api/ticker/CMM/%s' % ccy)
        return { ccy : Decimal(json['last'])}

class BtcMarkets(ExchangeBase):
    # BtcMarkets - Australian Exchange - AUD
    def get_rates(self, ccy):
        json = self.get_json('api.btcmarkets.net', '/market/CMM/%s/tick' % ccy)
